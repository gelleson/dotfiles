# Oh My Zsh's web-search plugin (ohmyzsh/ohmyzsh@74965c9), ported plugin-free.
# `google some words` opens the search in your browser; a bare `google` opens
# the home page. omz_urlencode is trimmed from OMZ's lib/functions.zsh.
...
function omz_urlencode() {
  ...
  local str="$@" url_str="" i byte LC_ALL=C
  export LC_ALL
  local dont_escape="[A-Za-z0-9"
  [[ -z $opts[(r)-r] ]] && dont_escape+=';/?:@&=+$,'
  # the mark set must be last because of the "-"
  [[ -z $opts[(r)-m] ]] && dont_escape+="_.!~*'()-"
  dont_escape+="]"
